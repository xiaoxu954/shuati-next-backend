...
import com.xiaoxu.shuati.mapper.QuestionBankMapper;
import com.xiaoxu.shuati.model.dto.questionBank.QuestionBankQueryRequest;
import com.xiaoxu.shuati.model.entity.QuestionBank;
import com.xiaoxu.shuati.model.entity.QuestionBankFavour;
import com.xiaoxu.shuati.model.entity.QuestionBankThumb;
import com.xiaoxu.shuati.model.entity.User;
import com.xiaoxu.shuati.model.vo.QuestionBankVO;
import com.xiaoxu.shuati.model.vo.UserVO;
import com.xiaoxu.shuati.service.QuestionBankService;
...
@Service
@Slf4j
public class QuestionBankServiceImpl extends ServiceImpl
        <QuestionBankMapper, QuestionBank> implements QuestionBankService {
...
// todo 从对象中取值
        ...
        List
                <String> tagList = questionBankQueryRequest.getTags();
    ...
    public QuestionBankVO getQuestionBankVO(QuestionBank questionBank, HttpServletRequest request) {
        ...
        if (loginUser != null) {
            // 获取点赞
            QueryWrapper
                    <QuestionBankThumb> questionBankThumbQueryWrapper = new QueryWrapper<>();
            ...
            // 获取收藏
            QueryWrapper
                    <QuestionBankFavour> questionBankFavourQueryWrapper = new QueryWrapper<>();
    ...
    @Override
    public Page
            <QuestionBankVO> getQuestionBankVOPage(Page<QuestionBank> questionBankPage, HttpServletRequest
            request) {
        List<QuestionBank> questionBankList = questionBankPage.getRecords();
        Page
                <QuestionBankVO> questionBankVOPage = new Page<>(questionBankPage.getCurrent(), questionBank
                Page.getSize(), questionBankPage.getTotal());
        if (CollUtil.isEmpty(questionBankList)) {
            return questionBankVOPage;
        }
        // 对象列表 => 封装对象列表
        List
                <QuestionBankVO> questionBankVOList = questionBankList.stream().map(questionBank -> {
        ...
        // 1. 关联查询用户信息
        Set
                <Long> userIdSet = questionBankList.stream().map(QuestionBank
        ...
        Map
                <Long
                        , Boolean> questionBankIdHasThumbMap = new HashMap<>();
        Map
                <Long
                        , Boolean> questionBankIdHasFavourMap = new HashMap<>();
        User loginUser = userService.getLoginUserPermitNull(request);
        if (loginUser != null) {
            Set
                    <Long> questionBankIdSet = questionBankList.stream().map(QuestionBank
            ...
            questionBankThumbQueryWrapper.eq("userId", loginUser.getId());
            List
                    <QuestionBankThumb> questionBankQuestionBankThumbList = questionBank
            ThumbMapper.selectList(questionBankThumbQueryWrapper);
            questionBankQuestionBankThumbList.forEach(questionBankQuestionBankThumb
                    -> questionBankIdHasThumbMap.put(questionBankQuestionBank
                    Thumb.getQuestionBankId(), true));
            // 获取收藏
            QueryWrapper
                    <QuestionBankFavour> questionBankFavourQueryWrapper = new QueryWrapper<>();
            ...
            questionBankFavourQueryWrapper.eq("userId", loginUser.getId());
            List
                    <QuestionBankFavour> questionBankFavourList = questionBank
            FavourMapper.selectList(questionBankFavourQueryWrapper);
            questionBankFavourList.forEach(questionBankFavour -> questionBank
                    IdHasFavourMap.put(questionBankFavour.getQuestionBankId(), true));
        }
        // 填充信息
        questionBankVOList.forEach(questionBankVO -> {
            ...
            questionBankVO.setHasThumb(questionBank
                    IdHasThumbMap.getOrDefault(questionBankVO.getId(), false));
            questionBankVO.setHasFavour(questionBank
                    IdHasFavourMap.getOrDefault(questionBankVO.getId(), false));
        });
        // endregion
